...
data_extract <- data_extract %>%
  ...
  mutate(
    split_id = rep(
      1:split_n, 
      each = ceiling(nrow(data_extract)/split_n), 
      length.out = nrow(data_extract)
      )
    ) %>%
  group_split(split_id) %>% 
  as.list()

data_anytest_sum <- list()
...
data_anytest_sum <- bind_rows(data_anytest_sum)
